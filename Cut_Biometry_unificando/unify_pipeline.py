...
def process_ficha(image_path: str, output_dir: str):
    ...
    # 2.1) Salvar classificação da ficha em class.txt
    class_file = os.path.join(output_dir, "class.txt")
    with open(class_file, "w") as f:
        f.write(classification)

    # 3) Marcação das colunas
    _, col1_coords, col2_coords = Colum_Marker.mark_columns(filtered_path, "", classification)
